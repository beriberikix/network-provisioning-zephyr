/*
 * Wi-Fi provisioning over the device console (shell) sample.
 *
 * Carries the provisioning protocol over a single shell command,
 *
 *     net_prov <endpoint> <session_id> <hex-request>
 *
 * — the transport esp_prov speaks with `--transport console`. On reboot, if
 * credentials are already stored, the sample connects with them instead —
 * retrying with backoff, and falling back to provisioning mode (credentials
 * kept) if the network stays unreachable.
 *
 * This is the transport used on boards that have Wi-Fi but no usable
 * Bluetooth/SoftAP path under Zephyr — most notably the Raspberry Pi Pico 2W
 * (CYW43439), where the in-tree drivers provide Wi-Fi STA only (no CYW43 BLE
 * HCI, and the AIROC driver has no concurrent SoftAP+STA). See the README.
 *
...
/* Proof-of-possession the client must supply to complete the security-1
 * handshake. Set to an empty string to advertise the "no_pop" capability.
 */
#define PROV_POP          "abcd1234"
/* The console scheme ignores the service name (no BLE name / AP SSID to set);
 * it is reported only in the proto-ver capabilities for parity with the other
 * transports.
 */
#define PROV_DEVICE_NAME  "PROV_ZEPHYR"
...
/* Example custom endpoint handler: echoes the request back to the client. */
		       ...
		       void *event_data)
{
	...
	case NETWORK_PROV_START:
		LOG_INF("Provisioning started; drive it from the console");
		LOG_INF("  shell command : net_prov <endpoint> <session_id> <hex>");
		LOG_INF("  esp_prov      : esp_prov --transport console "
			"--sec_ver 1 --pop %s ...", PROV_POP[0] ? PROV_POP : "(none)");
...
/* Run one full provisioning cycle: start the console transport, wait for
 * credentials, then stop.
 */
static int run_provisioning(void)
{
	 ...
	 * (CONFIG_NETWORK_PROV_AUTOSTOP_TIMEOUT_MS), so the client still has
	 * time to poll GetWifiStatus before the transport goes away.
	 */
...
int main(void)
{
	struct network_prov_mgr_config config = {
		 ...
		 * the client (upstream example default).
		 */
	...
	(void)network_prov_mgr_set_app_info("wifi_prov_console", "1.0", app_caps,
					    ARRAY_SIZE(app_caps));
	...
	if (provisioned) {
		...
		LOG_INF("Device not provisioned, starting console provisioning");
	}
